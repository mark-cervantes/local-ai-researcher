...
### Acceptance Criteria
- [ ] A default `read` call against the configured self-hosted reader returns full extracted content as the normal success behavior.
- [ ] When the caller explicitly requests a shorter payload, the response clearly identifies the selected truncation or excerpt mode.
- [ ] When a hard limit forces truncation, the response states that truncation happened, why it happened, and what limit was applied.
- [ ] Reader responses keep the shared metadata and provenance contract needed for later schema freeze.
- [ ] `gather` can rely on the same default read behavior for fetched pages without introducing a second content policy.
